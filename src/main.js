...
new Vue({
    ...
    data: {
        ...
        defaultParams: {
            posts: {
                _fields: "author,content,date,id,excerpt,slug,sticky,title",
                per_page: 5,
            },
            pages: {
                _fields:
                    "author,content,date,id,excerpt,slug,sticky,title,menu_order",
            },
    ...
    methods: {
        fetchPage(resource) {
            ...
            return axios.get(resource, { baseURL, params }).then((result) => {
                ...
                this.hasMore[resource] =
                    result.headers["x-wp-total"] > this[resource].length;
            });
        },
        fetchAll(resource) {
